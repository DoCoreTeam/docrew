...
**다른 AI 도구는 엔터를 누르는 순간 코드부터 쏟아냅니다.** DOMANGCHA는 문제를 완전히 이해하기 전까지 단 한 줄도 쓰지 않습니다. 그다음 플래너(Planner), 빌더(Builder), 평가자(Evaluator)를 병렬로 투입하고, 검토를 통과하지 못한 결과물은 모조리 막아냅니다.

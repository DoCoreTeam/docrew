...
**Other AI tools start coding the second you press Enter.** DOMANGCHA refuses to write a single line until it understands the problem. Then it deploys planners, builders, and evaluators in parallel — and blocks anything that fails review.
...
**당신이 엔터를 치는 그 0.3초, 다른 AI는 이미 틀린 코드 200줄을 짜고 있습니다.** DOMANGCHA는 다릅니다 — 코드 한 줄 쓰기 전에 "잠깐, 질문이 있어요"부터 시작합니다. 그다음 플래너, 빌더, 평가자를 전부 병렬로 굴리고, 검토 못 통과한 건 가차없이 반려합니다.

- **코드 전에 최대 12개 질문.** 귀찮으시죠? 그게 당신의 스프린트를 살립니다.
- **16명의 전문가, 명령 하나.** 채용 공고, 면접, 연봉 협상 전부 필요 없습니다. 비즈니스, 리서치, OSS 스카우팅, 풀스택, DevOps, 모바일, 통합, 문서, 카피, SEO, QA, 보안, 코드 리뷰, 토큰 예산까지.
- **5개의 게이트(Gate). "이번 한 번만"은 없습니다.** 300줄 초과 파일은 자동 차단, 버전 고정, 역할 분리, 기준 검증, 파괴적 변경 승인 — 전부 통과해야 세상에 나옵니다.
- **빌더 ≠ 리뷰어. 항상.** 자기 코드 자기가 리뷰하는 건 자기 요리 자기가 별점 매기는 것과 같습니다.
- **실수해도 됩니다. 단, 두 번은 안 됩니다.** 모든 실수는 즉시 게이트(Gate) 패턴으로 등록되어 영원히 반복되지 않습니다.

...
■ PHASE 0.1 — TRADEOFF CHECK (MEDIUM+ 전용 — SIZE ASSESSMENT 직후 실행):

  아래 체크리스트 스캔 (1차 자동):
  □ 기존 코드 대규모 수정 (500줄+ 영향) — 회귀 리스크
  □ 단기 해결책이 장기 아키텍처에 충돌
  □ 새 의존성 추가 → 보안/유지보수 비용 증가
  □ 요청 방식이 더 많은 버그/복잡도 생성 가능
  □ 기존 동작의 브레이킹 체인지 내포
  □ 환경 제약으로 실제 작동 불가 (예: ANSI 터미널 미지원)

  2차: CEO 자율 판단 → 이익 vs 비용 종합

  해당 없음 → [TRADEOFF CLEAR] ✅ → STACK SELECTION (PHASE 0.3) 진행
  해당 있음 → 출력 후 대기:
  [TRADEOFF DETECTED]
  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ⚠️  구현 가능하나 이런 문제가 더 생길 수 있습니다:
    • <리스크 1>
    • <리스크 2>
  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  선택지:
